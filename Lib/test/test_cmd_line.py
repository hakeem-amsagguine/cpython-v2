...
class CmdLineTest(unittest.TestCase):
    ...
    def test_stdout_flush_at_shutdown(self):
        ...
        self.assertIn(
            b"Exception ignored on flushing sys.stdout:\n"
            b"OSError: ".replace(b"\n", os.linesep.encode()),
            err,
        )
    ...
    @unittest.skipUnless(
        sysconfig.get_config_var("Py_TRACE_REFS"),
    ...
    def test_python_dump_refs_file(self):
        with tempfile.NamedTemporaryFile() as dump_file:
            code = "import sys; sys._clear_type_cache()"
            rc, out, err = assert_python_ok(
                "-c", code, PYTHONDUMPREFSFILE=dump_file.name
            )
            self.assertEqual(rc, 0)
    ...
    @unittest.skipUnless(
        sys.platform == "darwin", "PYTHONEXECUTABLE only works on macOS"
    )
    ...
    @unittest.skipIf(
        "-fsanitize" in sysconfig.get_config_vars().get("PY_CFLAGS", ()),
    ...
    @unittest.skipUnless(
        ...
        "Requires HAVE_PERF_TRAMPOLINE support",
    )
    ...
    def test_cpu_count_default(self):
        ...
        res = assert_python_ok(
            "-X", "cpu_count=default", "-c", code, PYTHON_CPU_COUNT="1234"
        )
        self.assertEqual(self.res2int(res), (os.cpu_count(), os.process_cpu_count()))
    ...
    def test_import_time(self):
        ...
        self.assertRegex(
            res.err.decode("utf-8"), r"import time: \s*\d+ | \s*\d+ | \s*os"
        )
        code = "import os; import os"
        res = assert_python_ok("-X", "importtime=2", "-c", code)
        self.assertRegex(
            res.err.decode("utf-8"), r"import time: cached\s* | cached\s* | os"
        )
